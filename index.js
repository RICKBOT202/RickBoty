...
const vcard = 'BEGIN:VCARD\n' 
                      ...
                      + 'FN:Rick\n' 
                      + 'ORG: Author Bot;\n' 
                      + 'TEL;type=CELL;type=VOICE;waid=6281938716903:+62 819-3871-6903\n' 
                      + 'END:VCARD' 
const vcard1 = 'BEGIN:VCARD\n' 
                      + 'VERSION:3.0\n' 
                      + 'FN:Bot Rick\n' 
                      + 'ORG: Author Bot;\n' 
                      + 'TEL;type=CELL;type=VOICE;waid=6288286820474:+62 882-8682-0474\n' 
                      + 'END:VCARD'
/********************END********************/ 
blocked = []   
prefix = '!'
limitawal = 100 //Terserah
memberlimit = 10
ator = 'subrek'
namo = 'busmaniaofficial_id'
cr = '*RICK BOT*\n*Youtube : busmaniaofficial_id*\n*Instagram : @busmaniaofficial_id*'
vz = '*TEST*'
/*****************************************/
/********** OWNER NUMBER**********/
const ownerNumber = ["6281938716903@s.whatsapp.net","6288286820474@s.whatsapp.net"] 
/*************************************/ 
/*********** LOAD FILE ************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('©','red'),color(']','white'),color('SCAN QR CODE !','white'),color('SUBSCRIBE','yellow'),color('busmaniaofficial_id','aqua'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const busmaniaofficial_id = " ~ by busmaniaofficial_id"
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izin admin")) return reply("#izin admin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`*[ ! ] 𝗪𝗔𝗥𝗡𝗜𝗡𝗚 𝗟𝗜𝗡𝗞 𝗚𝗥𝗢𝗨𝗣 𝗗𝗘𝗧𝗘𝗖𝗧𝗢𝗥*\nᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇ\nʟɪɴᴋ ɢʀᴏᴜᴘ ᴛᴇʀᴅᴇᴛᴇᴋꜱɪ ɪᴍ ꜱᴏʀʀʏ ᴛᴏ ᴍᴀᴀꜰ\n${sender.split("@")[0]} ʏᴏᴜ ᴋᴀᴍᴜ ᴀᴋᴀɴ ᴅɪ ᴋᴇʟᴜᴀʀᴋᴀɴ/ᴅɪᴋɪᴄᴋ ᴅᴀʀɪ ɢʀᴏᴜᴘ ᴅᴀʟᴀᴍ 2 ᴅᴇᴛɪᴋ`)
 	     setTimeout( () => {
		...
		case 'nulis2': 				
				...
				reply(mess.wait)
					...
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, busmaniaofficial_idganss, ` ~ Harta Tahta ${args[0]}`)
					} else {
...
break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    client.sendMessage(from, buffer, image, {caption: 'Jangan Cuma Make Doang Follow instagram.com/busmaniaofficial_id', quoted: mek})
         break
         case 'gplaybutton':
         case 'splaybutton':
                if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} busmaniaofficial_id`)
                    txt = args.join(" ")
                    reply('[❕] Loading')
         ...
         break
				...
				case 'beritahoax':
					...
					if (anu.error) return reply(anu.error)
					teks = `â *Title* : ${anu.title} \nâ *Size* : ${anu.filesize} \nâ *Desc* : ${anu.desc} \n\n*VIDEO SEDANG DIKIRIMKAN, JANGAN SPAM YA Yank...*`
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`Contoh : ${prefix}googletext busmaniaofficial_id/gans/banget`)
					reply(ind.wait())
		 ...
		 case 'report':
                ...
                const teks1 = `*[REPORT]*\nNumber : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${pesan}`
                    ...
                    client.sendMessage('6281938716903@s.whatsapp.net', options, text, {quoted: mek})
     ...
     case 'request':
					...
					client.sendMessage('6281938716903@s.whatsapp.net', options, text, tescuk, vz, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER RICK BOT, Request Palsu Atau Main² Tydak Akan Ditanggapi Terimakasih.')
					break
				case 'listhargadmff':
				case 'listdiamondff':
				if (!isRegistered) return reply(ind.noregis())
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Jangan Lupa Di Follow yaa : instagram.com/busmaniaofficial_id',MessageType.text, { quoted: mek} )
				  break    
			case 'leaderboard':
				case 'lb':
				bo = args[0]
...
⌬ Limit: ${limitawal-updated.limit}`
		...
		if (!isGroup) return reply(ind.groupo())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`Contoh : ${prefix}glitchtext busmaniaofficial_id/gans`)
					reply(ind.wait())
				...
				try {
						...
						let { user, stats } = await tikYank.getUserProfileInfo(args[0])
						reply(ind.wait())
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘀𝗶𝗺𝗶 𝗱?? 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻??️️')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'tagall':
					...
					mentions('╔══🜲〘 OTAG ALL 〙🜲══'+teks+'╚═〘 *RICK BOT* 〙', members_id, true)
					break
				case 'delete':
				case 'del':
				case 'd':
				...
				case 'clone':
					...
					var yogi = gh.split("/")[0];
					var cmnt = gh.split("/")[1];
					if (args.length < 1) return reply(`Contoh : ${prefix}phcomment busmaniaofficial_id/yt`)
					reply(ind.wait())
				...
				if (anu.error) return reply(anu.error)
				infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA Yank*`
				...
				if (budy.includes(`Yank`)) {
                  reply(`Iya Yank, aku disini ada yg bisa dibantu?`)
                  }
				...
				if (budy.includes(`Pagi`)) {
                  reply(`Iya pagi Yank, udah senyum belum hari ini?`)
                  }
                  
				if (budy.includes(`Malem`)) {
                  reply(`Iya malem Yank, udah senyum belum hari ini?`)
                  }
				...
				if (budy.includes(`Halo`)) {
                  reply(`Halo Yank, bot disini ada yg bisa dibantu?`)
                  }
                  
				if (budy.includes(`Hai`)) {
                  reply(`Hai juga Yank, bot disini ada yg bisa dibantu?`)
                  }
		...
		if (budy.includes(`Bot`)) {
                  reply(`Iya Yank, bot disini ada yg bisa dibantu?`)
                  }
		...
		if (budy.includes(`makasih`)) {
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[RICK]','yellow'), 'Unregistered Command from', color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
